...
target_report <- function(data, obs, mod, point,
  ...
  Mod.ave=Obs=Obs.ave=Point=bias_norm=crmse_norm=n_valid=rmsu=NULL
  ...
  Dat %>% group_by(Point) %>%
    summarize(Obs.ave = mean(Obs, na.rm=T),
              ...
              n_valid = n()) %>%
    filter(n_valid>=365*0.75) -> quality_points
  quality_overall <- data.frame(mqi_ts_p90    = quantile(quality_points$mqi_ts, probs=0.9),
                                mqi_year_p90  = quantile(quality_points$mqi_year, probs=0.9),
  ...
  out$parameters$RV <- threshold(pollutant)
...
#'
#' @import ggplot2
#' @return \code{target_plot} returns a a target plot (object of class \code{ggplot} and \code{gg})
#' @rdname target_plot
#' @export
target_plot <- function(t_rep) {
  Point=bias_norm=crmse_norm=NULL
  ggplot(data = t_rep$quality_points,
         aes(x = crmse_norm, y = bias_norm, shape = Point)) +
    ...
    stat_function(fun=function(x){+sqrt(1-x^2)}, geom="area", aes(fill="chartreuse3"), n=1001, alpha=0.4, col=NA)+
    stat_function(fun=function(x){-sqrt(1-x^2)}, geom="area", aes(fill="chartreuse3"), n=1001, alpha=0.4, col=NA)+
    ...
    stat_function(fun=function(x){+sqrt((1/t_rep$parameters$beta)^2-x^2)}, geom="line",
                  colour="darkgrey", n=2001, linetype=3)+
    stat_function(fun=function(x){-sqrt((1/t_rep$parameters$beta)^2-x^2)}, geom="line",
                  colour="darkgrey", n=2001, linetype=3)+
    ...
    xlab(expression(CRMSE[norm]))+ylab(expression(BIAS[norm])) -> p
  p +
    ...
    annotate(geom = "text", x = -2, y =  1.8, size=6, parse=T,
             label = paste0("MQI==",signif(t_rep$quality_overall$mqi_ts_p90,3)),
             color = ifelse(signif(t_rep$quality_overall$mqi_ts_p90,3)>1,
                            "firebrick3","green4"), adj=0, alpha=0.8)+
    annotate(geom = "text", x = -2, y =  1.6, size=4, parse=T,
             label = paste0("MQI[year]==",signif(t_rep$quality_overall$mqi_year_p90,3)),
             color = ifelse(signif(t_rep$quality_overall$mqi_year_p90,3)>1,
                            "firebrick3","green4"), adj=0, alpha=0.8) +
